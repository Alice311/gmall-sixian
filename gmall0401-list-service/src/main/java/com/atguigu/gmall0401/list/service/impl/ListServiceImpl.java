...
public class ListServiceImpl implements ListService {
    ...
    @Autowired
    RedisUtil redisUtil; //做redis操作，要有客户端程序
	...
	//index的使用前提：创建builder:
	//builder里的四件东西：数据：skuLsInfo, 索引名：gmall0401_sku_info
	//type名：——doc, id: skuLsInfo.getId()
        ...
        try {
            jestClient.execute(index);//index插入
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    // 页行数
        ...
        System.out.println(searchSourceBuilder.toString());//应该和字符串一模一样
