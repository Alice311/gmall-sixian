...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    @Transactional//涉及两个表。在GmallOrderServiceApplication 加@EnableTransactionManagement
    public String saveOrder(OrderInfo orderInfo) {

	/*
	先保存主表，才有主key.
	在OrderInfo里有@GeneratedValue，可以把id回写回来
	*/
	...
	//前提：@AutoWired redisUtill....
        Jedis jedis = redisUtil.getJedis();
	
	//临时性的东西必须设置过期时间。不然关闭之后这个就用不到了，还需要清除
	...
	*/
	...
	/*
	目的：改状态——改ProcessStatus processStatus的值
			——需要放参数-> new OrderInfo()
	准备orderInfoMapper
	orderinfo中放入需要改的值
		处理状态
	...
	*/
	...
	得到订单信息
	循环订单
		前提：得到结构<-看接口得到的wareskuMap, 是个List<Map>
