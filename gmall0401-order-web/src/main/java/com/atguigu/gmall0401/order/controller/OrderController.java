...
public class OrderController {
	...
	//token不可用
        if(!isEnableToken){
            ...
            return  "tradeFail";。。返回tradeFail页面
        }
        ...
        orderInfo.setExpireTime(DateUtils.addMinutes(new Date(),15));//过期时间
   ...
   public String checkSkuNum(OrderDetail orderDetail){
	...
	/*
	其他方法：orderDetailList每个遍历一次hasStock
	调用远程，单词消耗时间长
	如果需要处理的商品多，时间就更长。
	...
	*/
	...
	/*
	CompletableFuture.supplyAsync()->后代表真正要执行的业务：检查checkNum
	whenComplete()完成操作：()第一个参数是前一步操作的结果ifPass,第二个参数是接收异常ex。
	num->->经过一下操作，最后变成 CompletableFuture<Boolean>
	*/
